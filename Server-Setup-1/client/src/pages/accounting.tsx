...
export default function AccountingPage() {
  ...
  const [reversalConfirmEntry, setReversalConfirmEntry] = useState<IGeneralLedgerEntry | null>(null);
  ...
  const reversalMutation = useMutation({
    mutationFn: async (entryId: string) => {
      const res = await apiRequest("POST", "/api/accounting/reversals", { entryId });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/accounting/ledger"] });
      queryClient.invalidateQueries({ queryKey: ["/api/accounting/accounts"] });
      setReversalConfirmEntry(null);
      toast({ title: "Entry reversed successfully" });
    },
    onError: (err: Error) => {
      setReversalConfirmEntry(null);
      toast({ title: "Failed to reverse entry", description: err.message, variant: "destructive" });
    },
  });

  const reversedEntryIds = new Set(
    ledgerEntries.filter((e) => e.isReversing && e.originalEntryId).map((e) => e.originalEntryId)
  );
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 overflow-auto h-full">
                  ...
                  <TableBody>
                    {filteredEntries.length === 0 ? (
                      ...
                      filteredEntries.map((entry) => {
                        const isReversed = reversedEntryIds.has(entry._id);
                        const isReversal = entry.isReversing;
                        return (
                          <TableRow key={entry._id} data-testid={`row-ledger-${entry._id}`} className={isReversal ? "text-red-500" : isReversed ? "line-through opacity-60" : ""}>
                            <TableCell className={isReversal ? "text-red-500" : "text-muted-foreground"}>{formatDate(entry.date)}</TableCell>
                            <TableCell className="font-medium">
                              {entry.accountName}
                              {isReversal && <Badge variant="destructive" className="ml-2 text-xs">Reversal</Badge>}
                              {isReversed && <Badge variant="outline" className="ml-2 text-xs">Reversed</Badge>}
                            </TableCell>
                            ...
                            <TableCell className={isReversal ? "text-red-500" : "text-muted-foreground"}>{entry.description || "-"}</TableCell>
                            <TableCell className="font-mono text-xs">{entry.referenceType ? `${entry.referenceType}:${entry.referenceId}` : "-"}</TableCell>
                            {isAdmin && (
                              <TableCell>
                                {!isReversal && !isReversed && (
                                  <Button variant="ghost" size="sm" onClick={() => setReversalConfirmEntry(entry)} data-testid={`button-reverse-${entry._id}`}>
      ...
      <Dialog open={!!reversalConfirmEntry} onOpenChange={(open) => { if (!open) setReversalConfirmEntry(null); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Confirm Reversal</DialogTitle>
            <DialogDescription>
              Are you sure you want to reverse this entry? This will create an opposite debit/credit entry.
            </DialogDescription>
          </DialogHeader>
          {reversalConfirmEntry && (
            <div className="text-sm space-y-1">
              <p><strong>Account:</strong> {reversalConfirmEntry.accountName}</p>
              <p><strong>Debit:</strong> {formatCurrency(reversalConfirmEntry.debit)}</p>
              <p><strong>Credit:</strong> {formatCurrency(reversalConfirmEntry.credit)}</p>
              <p><strong>Description:</strong> {reversalConfirmEntry.description || "-"}</p>
            </div>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setReversalConfirmEntry(null)} disabled={reversalMutation.isPending}>
              Cancel
            </Button>
            <Button
              variant="destructive"
              onClick={() => reversalConfirmEntry && reversalMutation.mutate(reversalConfirmEntry._id)}
              disabled={reversalMutation.isPending}
              data-testid="button-confirm-reversal"
            >
              {reversalMutation.isPending && <Loader2 className="animate-spin mr-1" />}
              Reverse Entry
            </Button>
          </DialogFooter>
        </DialogContent>
